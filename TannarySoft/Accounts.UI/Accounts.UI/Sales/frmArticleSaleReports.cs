...
{
    public partial class frmArticleSaleReports : MetroForm
    {
        frmFindAccounts frmaccounts = null;
        frmStockAccounts frmstockAccounts = null;
        public string ReportType { get; set; }
        Guid IdArticle;
        public frmArticleSaleReports()
        {
            InitializeComponent();
        }
        ...
        private void FillComboDetail()
        {
            ...
            ddlReportTypes.Items.Insert(1, "Maker Sale Report");
            ddlReportTypes.Items.Insert(2, "Articles Sales Report");
            ddlReportTypes.Items.Insert(3, "Articles Total Stock Report");
            ddlReportTypes.Items.Insert(4, "Articles DateWise Total Stock Report");
        ...
        private void GetBindingListByCode(short BindingListCode, string AccountNo)
        {
            ...
            if (BindingListCode == 1 && AccountNo != string.Empty)
            {
                ...
                {
                    listSale = SaleManager.GetMakerSaleReportByDate(AccountNo, dtPrescriberStart.Value, dtPrescriberEnd.Value, Operations.IdCompany);
                }
                else
                {
                    listSale = SaleManager.GetMakerReportSale(Validation.GetSafeLong(AccountNo), Operations.IdCompany);
                }
            }
            else if (BindingListCode == 2 && AccountNo != string.Empty)
            {
                ...
                {
                    listSale = SaleManager.GetArticleDetailSaleReportByDate(IdArticle, dtProductStart.Value, dtProductEnd.Value, Operations.IdCompany);
                }
                else
                {
                    listSale = SaleManager.GetArticleDetailSaleReport(IdArticle, Operations.IdCompany);
                }
            }
            else if (BindingListCode == 3)
            {

                listSale = SaleManager.GetArticlesTotalSalesReport(Operations.IdCompany);
            }
            else if (BindingListCode == 4)
            {
                listSale = SaleManager.GetArticlesTotalSaleByDate(dtStartDateWiseStock.Value, dtEndDateWiseStock.Value, Operations.IdCompany);
            }
