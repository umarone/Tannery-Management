    ...
    public partial class frmGatePassReports : MetroForm
    {
        ...
        private void GetBindingListByCode(short BindingListCode, string AccountNo)
        {
            ...
            if (BindingListCode == 1 && AccountNo != string.Empty)
            {
                ...
                {
                    list = manager.GetEmployeeIssuanceReportByDate(AccountNo, dtPrescriberStart.Value, dtPrescriberEnd.Value, Operations.IdCompany);
                }
                else
                {
                    list = manager.GetEmployeeIssuanceReport(AccountNo, Operations.IdCompany);
                }
            ...
            else if (BindingListCode == 2 && AccountNo != string.Empty)
            {
                ...
                {
                    list = manager.GetGeneralProductDetailIssuanceByDate(Validation.GetSafeLong(AccountNo), dtProductStart.Value, dtProductEnd.Value, Operations.IdCompany);
                }
                else
                {
                    list = manager.GetGeneralProductDetailIssuance(Validation.GetSafeLong(AccountNo), Operations.IdCompany);
                }

            }
            else if (BindingListCode == 3)
            {
                list = manager.GetGeneralProductsTotalIssuance(Operations.IdCompany);
            }
            else if (BindingListCode == 4)
            {

                list = manager.GetGeneralProductsTotalIssuanceByDate(dtStartDateWiseStock.Value, dtEndDateWiseStock.Value, Operations.IdCompany);
        ...
        private void BindIssuanceGridByCode(List<SaleDetailEL> list, short BindingCode)
        {
